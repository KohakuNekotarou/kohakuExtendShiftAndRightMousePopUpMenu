...
/** DynMnuDynamicMenu
	implements the IDynamicMenu interface.
	Builds the menu each time it is exposed.
	メニューが公開されるたびに構築します。
...
class DynMnuDynamicMenu : public CPMUnknown<IDynamicMenu>
{
	public:
		...
		*/
		...
		/** 
			This method controls what menu items appear at any given time.
			この方法は、任意の時点で表示されるメニュー項目を制御します。

			InDesign calls this method when the menu is exposed and any menu entry in the MenuDef resource uses the kSDKDefIsDynamicMenuFlag.
			InDesignは、メニューが表示された際にこのメソッドを呼び出します。また、MenuDefリソース内のいずれかのメニュー項目がkSDKDefIsDynamicMenuFlagを使用している場合にも呼び出されます。

			Control is exercised by rebuilding this action component's valid menus with the menu and action managers.
			制御は、メニューおよびアクションマネージャを用いてこのアクションコンポーネントの有効なメニューを再構築することで行われる。

			This example implementation builds the menus and action IDs based on the number of spreads in the front document.
			この実装例では、フロントドキュメントのスプレッド数に基づいてメニューとアクションIDを構築します。

			@param dynamicActionID ID of menu hit just before this method is called.
			　　　　　　　　　　　このメソッドが呼び出される直前にヒットしたメニューのID。
		...
		　　　　　　　　　　　　　このアクションを呼び出したウィジェットを含みます。nilの場合があります。例：パネルのポップアップメニューが押された後にパネル内のウィジェットを検索するUtils<IWidgetUtils>()->QueryRelatedWidget(...)の基盤として有用です。
		*/
...
bool16 DynMnuDynamicMenu::SiftAfterFlg = kFalse;
...
void DynMnuDynamicMenu::RebuildMenu(ActionID dynamicActionID, IPMUnknown* widget)
{
	do {
		...
		{
			// 通常メニューが表示された後か
			if (DynMnuDynamicMenu::SiftAfterFlg == kFalse)
			{
				for (int32 i = 0; i < TestMenuFilter::vector_ActionID_actionID.size(); i++)
				{
					menuMgr->RemoveMenuItem(
						TestMenuFilter::vector_PMString_menuPath[i], // Menu path
						TestMenuFilter::vector_ActionID_actionID[i] // ActionID
					);
				}
				DynMnuDynamicMenu::SiftAfterFlg = kTrue;
		...
		{
			// シフトメニューが表示された後か
			if (DynMnuDynamicMenu::SiftAfterFlg == kTrue)
			{
				menuMgr->RemoveMenuItem(
					"RtMouseDefault", // Menu path
					kDynMnuStaticMenuActionID // ActionID
				);

				for (int32 i = 0; i < TestMenuFilter::vector_ActionID_actionID.size(); i++)
				{
					menuMgr->AddMenuItem(
						TestMenuFilter::vector_ActionID_actionID[i], // Action ID
						TestMenuFilter::vector_PMString_menuPath[i], // Menu path
						TestMenuFilter::vector_PMReal_menuPos[i], // Menu position
						kFalse // IsDynamicMenu
					);
				}
				DynMnuDynamicMenu::SiftAfterFlg = kFalse;
			}
		}
		...
		// This list just keeps track of what we're displaying -- so we don't remove something that isn't displayed.
		// このリストは表示しているものだけを追跡します――つまり、表示されていないものを削除しないようにするためです。
		// As a bonus it also maps an action ID to a UID.
		// おまけとして、アクションIDをUIDにマッピングします。
